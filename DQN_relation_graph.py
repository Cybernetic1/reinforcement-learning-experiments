"""
DQN (Deep Q Network) using my invention "relation graph network" to implement Q.
...
class RG_Network(nn.Module):
	...
	def __init__(self, input_dim, action_dim, hidden_size, activation=F.relu, init_w=3e-3):
		super(RG_Network, self).__init__()
	...
	def match(γ, x, y):
		match_degree = RG_Network.sigmoid(γ) * (x - y)**2
		return match_degree
	...
	def forward(self, state):
		...
		for m in range(0, self.M):			# for each rule
			...
			for j in range(0, self.J):		# for each atom in rule
				...
				logits = RG_Network.softmax(weights)
...
class DQN():
	...
	def __init__(
		...
		hidden_dim = 9
		self.rg_net = RG_Network(state_dim, action_dim, hidden_dim, activation=F.relu).to(device)

		self.q_criterion = nn.MSELoss()
		self.q_optimizer = optim.Adam(self.rg_net.parameters(), lr=self.lr)

	def choose_action(self, state, deterministic=True):
		state = torch.FloatTensor(state).unsqueeze(0).to(device)
		logits = self.rg_net(state)
	...
	def update(self, batch_size, reward_scale, gamma=0.99):
		...
		logits = self.rg_net(state)
		next_logits = self.rg_net(next_state)
